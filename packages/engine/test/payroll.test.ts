...
describe("YTD gross is prior-YTD + this month (DB-summed prior, not wage×month)", () => {
  test("prior YTD accumulates onto this month's gross", () => {
    const r = calculatePayroll(input({ monthlySalary: 3_750, priorYtdGross: 18_000 }));
    expect(r.ytdGross).toBe(21_750); // 18,000 prior + 3,750 — the exact bug being fixed
  });
});
...
describe("FUTA wage base ($7,000) — driven by prior-YTD gross", () => {
  ...
  test("partial FUTA when this month straddles the $7,000 base", () => {
    // prior YTD 3,500; only 3,500 of the base remains → 3,500 * 0.006 = 21.00
    const r = calculatePayroll(input({ monthlySalary: 3_500, priorYtdGross: 3_500 }));
    expect(r.employerFUTA).toBe(21.0);
  });
});
...
describe("W-4 exempt from federal withholding", () => {
  // Neilson Soult's W-4 filed 2026-03-17 elects EXEMPT for the 2026 calendar
  // year (Second Brain #2009). At $3,750/mo the on-file paystub numbers are:
  //   Federal W/H $0.00, SS $232.50 (3,750*.062), Medicare $54.38 (3,750*.0145),
  //   Total deductions $286.88, Net $3,463.12 — all computed by hand here.
  const exempt = calculatePayroll(input({ monthlySalary: 3_750, federalExempt: true }));
  const normal = calculatePayroll(input({ monthlySalary: 3_750, federalExempt: false }));
  ...
  test("default (non-exempt) still computes withholding from brackets", () => {
    // Annual taxable = 45,000 - 16,100 = 28,900.
    // Fed tax = 12,400*.10 + 16,500*.12 = 1,240 + 1,980 = 3,220 → /12 = 268.33
    expect(normal.federalWithholding).toBe(268.33);
  });

  test("exempt zeroes ONLY federal — FICA and employer taxes unchanged", () => {
    expect(exempt.socialSecurity).toBe(232.5); // 3,750 * 0.062
    expect(exempt.medicare).toBe(54.38); // 3,750 * 0.0145
  ...
  test("exempt total deductions $286.88 and net pay reconcile to the paystub", () => {
    expect(exempt.totalDeductions).toBe(286.88); // 0 + 232.50 + 54.38
    expect(exempt.netPay).toBe(3_463.12); // 3,750.00 − 286.88
    // Net pay and total deductions reconcile exactly to gross — no per-field
    // rounding drift (54.375 → 54.38 is the half-cent case).
