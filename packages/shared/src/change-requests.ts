 ...
 * allowances, per-period extra, exempt flag. Exempt supersedes the worksheet:
 * an exempt election cannot also claim allowances or extra withholding
 * (same rule as the admin election route).
 */
export const stateElectionPayload = z
  .object({
  ...
  .refine((v) => !v.exempt || (v.allowances === 0 && v.extraWithholding === 0), {
