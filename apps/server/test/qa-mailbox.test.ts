...
const MAILPIT_MESSAGES = {
  messages: [
    {
      ID: "msg2",
      Subject: "SOULT IO LTD Payroll — test email",
    ...
    {
      ID: "msg1",
      Subject: "SOULT IO LTD Payroll — payslip issued",
      From: { Address: "payroll@example.test" },
...
const MAILPIT_DETAIL = {
  ...
  Text: "Test email from SOULT IO LTD Payroll admin settings. SMTP delivery is working.",
  HTML: "<p>Test email…</p>",
};
...
describe("GET /api/qa/mailbox (APP_ENV=qa)", () => {
  ...
  it("returns the latest message to an address with subject/to/text", async () => {
    ...
    expect(body.subject).toBe("SOULT IO LTD Payroll — test email");
