...
- **Bun is genuinely production-viable in 2026** (1.3 shipped builtin Postgres/MySQL/Redis clients; ~98% npm compat) — but the recurring caveat from production case studies is *intermittent memory leaks and long-running-process stability vs. V8*. For a payroll service that must run unattended for months on a home server, that's disqualifying at the margin, not the speed.
- **Go's PDF ecosystem is its one real gap**: `jung-kurt/gofpdf` was archived in 2021 (the `phpdave11/gofpdf` fork and Maroto v2 carry on), and HTML→PDF means headless Chromium (Rod/chromedp) — a 300–500 MB spike you don't want on a home server. For *tabular payslips generated on demand*, canvas-style generation is actually fine.
- **.NET is the dark horse**: first-class `decimal`, Hangfire, QuestPDF — it scored 89 despite footprint. If the app ever moves off the home server to a real VPS, it becomes arguably the best payroll stack.
- **Postgres-backed job queues matured everywhere** (River for Go, Oban for Elixir, pg-boss for Node, Hangfire for .NET) — no Redis needed for the cron-like payroll schedule on any stack. This removes Node's main home-server disadvantage.
...
**Flip condition:** if "don't touch the math" outweighs "optimal home-server residency," take Node and never look back.
...
| Risk | Mitigation |
|---|---|
| **PDF generation is the weakest ecosystem link** (gofpdf archived; Chromium too heavy for home server) | Payslips are fixed-layout tables — use `phpdave11/gofpdf` or **Maroto v2** (actively maintained). Alternative: shell out to the **Typst** single binary (~30 MB) for templated PDFs — a clean escape hatch. |
