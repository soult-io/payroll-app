/**
 * Legacy migration CLI (spec 9): copies payroll history from an external
 * legacy database (`accounting` schema) into the payroll database.
 *
 * Usage:
 ...
 *   postgres://<user>:<pw>@<legacy-host>:5432/<legacy-db>
 *
...
if (!sourceUrl) {
  console.error(
    "error: SOURCE_DATABASE_URL is required (the legacy source postgres connection string)",
  );
  process.exit(1);
}
if (!dryRun && !write) {
