 ...
 * (an external legacy database, `accounting` schema) plus synthetic issued runs.
 *
 * The DDL below mirrors the legacy source schema (final post-migration shape:
 * payroll_runs header-only with employee_id, w4_elections with
 * effective_from). Seed values are SYNTHETIC fixture data — sole employee
 * Ada Lovelace / Example Corp, the 2025→2026-03 $4,000/mo and 2026-04+
 * $4,250/mo compensation schedule, the 2026 exempt W-4 effective 2026-04-01,
 * and the 2025 + 2026 statutory tables.
 *
 * Synthetic runs (2025 Jan–Dec, 2026 Jan–Jul) are computed by the VENDORED
 * engine itself (@payroll/engine) and mimic a plausible legacy history,
 * including Q1-2026 quirks:
 *   - 2026-01 / 2026-02 were issued with the WRONG (2025) tax tables, so this
 *     fixture computes them with TAX_CONFIG_2025.
 *   - 2026-03 was trued up against a filed Q1-2026 Form 941: the stored
 *     federal_withholding is $490.73 ($370.33 March + $60.20 + $60.20 Jan/Feb
 *     corrections) and net_pay $3,203.27 — NOT the engine's $298.33/$3,395.67.
...
CREATE TABLE accounting.deposits (
  ...
  note            TEXT DEFAULT '',
  created_at      TIMESTAMPTZ DEFAULT now(),
  created_by      TEXT DEFAULT 'legacy',
...
// ---------------------------------------------------------------------------
// Synthetic seed values (fixture constants; no real personal or payroll data)
// ---------------------------------------------------------------------------

export const LEGACY_EMPLOYEE = { fullName: "Ada Lovelace", entity: "Example Corp" } as const;

export const LEGACY_COMPENSATION = [
  { monthlySalary: 4000, effectiveFrom: "2025-01-01", effectiveTo: "2026-03-31" },
  { monthlySalary: 4250, effectiveFrom: "2026-04-01", effectiveTo: null },
] as const;
...
export async function createSourceFixture(
  ...
  // ---- reference data (the synthetic seed values) ----
  ...
  if (opts.withRuns ?? true) {
    ...
    for (const { year, month } of FIXTURE_RUN_PERIODS) {
      ...
      // regeneration: stub itemizes $370.33 (March at the filed
      // monthly rate) + $60.20 + $60.20 (Jan/Feb corrections) = $490.73.
      const isMarchTrueUp = year === 2026 && month === 3;
      const storedFederal = isMarchTrueUp ? 490.73 : result.federalWithholding;
